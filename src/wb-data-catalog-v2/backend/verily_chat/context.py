...
def _format_column(name: str, tc: Optional[dict], sc: Optional[dict]) -> str:
    ...
    header = f"  {name} ({dtype}){definition}{sensitivity}{uom}"

    # Pack all stats into one dense line
    parts: list[str] = []
    if tc:
        null_pct = tc.get("null_percent")
        dc = tc.get("distinct_count")
        if null_pct is not None or dc is not None:
            frags = []
            if null_pct is not None:
                frags.append(f"{null_pct}% null")
            if dc is not None:
                frags.append(f"{dc:,} distinct")
            parts.append(", ".join(frags))

        if tc.get("pattern"):
            parts.append(tc["pattern"])

        nstats = tc.get("numeric_stats") or {}
        if nstats.get("min") is not None:
            r = f"[{nstats['min']}..{nstats['max']}]"
            if nstats.get("median") is not None:
                r += f" med={nstats['median']}"
            parts.append(r)
        ...
        if top:
            if vcounts:
                ...
                items = [str(v) for v in top[:5]]
            parts.append("top: " + ", ".join(items))

        if tc.get("anomalies"):
            parts.append("!" + ",".join(tc["anomalies"]))

    if sc:
        terms = sc.get("terminology_bindings", [])
        if terms:
            parts.append("term:" + ",".join(t.get("system", "").split("/")[-1] for t in terms[:3]))

        joins = sc.get("join_paths", [])
        if joins:
            parts.append("joins:" + ",".join(joins[:3]))
