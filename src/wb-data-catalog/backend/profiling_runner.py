...
from bq_discovery import discover_bq_tables
from gcs_paths import parse_fq_table, sem_object_path, tech_object_path
from gcs_reader import read_json_if_exists
from gcs_writer import write_sem_profile, write_tech_profile
from profiler.models import BQTableInfo
from profiler.prompt_engine import detect_available_model
from profiler.semantic_profiler import profile_table_semantic
from profiler.tech_profiler import profile_table
...
def load_table_info(
    ...
    if p != data_project:
        return None
    tables = discover_bq_tables(p, d, billing_project)
    for ti in tables:
        if ti.table_id == t:
            return ti
    return None
    ...
    *,
    ...
    def _work() -> None:
        try:
            ...
            prof = profile_table(info, billing_project)
            write_tech_profile(bucket, fq_table, prof.to_json_dict(), billing_project)
...
def _dict_to_tech_profile(d: dict[str, Any]) -> Any:
    from profiler.models import TechColumnProfile, TechTableProfile, ValidationResult
    ...
    *,
    ...
    def _work() -> None:
        try:
            ...
            sem = profile_table_semantic(tech_prof, model, billing_project)
            write_sem_profile(bucket, fq_table, sem.to_json_dict(), billing_project)
        except Exception as e:
