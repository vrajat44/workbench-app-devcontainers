...
from profiler.prompt_engine import call_gemini, extract_json_from_response
...
1. "business_name": a short, human-friendly name for the table (e.g. "County Natality by Father Race")
2. "table_definition": 2–3 sentence plain-language description of what this table contains,
   its purpose, and how it relates to the broader dataset
3. "columns": a JSON array where each element has:
...
def _parse_llm_output(raw: Optional[dict | list], tech_profile: TechTableProfile) -> tuple[dict, dict]:
    """Parse LLM JSON response into (table_meta, column_name -> dict mapping)."""
    if raw is None:
        return {}, {}
