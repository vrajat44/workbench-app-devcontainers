"""
BigQuery dataset and table discovery (same pattern as WB_Data_Profiler).
"""
...
from profiler.models import BQColumnInfo, BQTableInfo
...
def discover_bq_tables(
    ...
    try:
        ...
        for row in table_rows:
            table_meta[row.table_name] = {"table_type": row.table_type, "row_count": None, "size_bytes": None}
    except Exception as e:
    ...
    try:
        ...
        """
        storage_rows = client.query(storage_sql).result()
        for row in storage_rows:
            if row.table_name in table_meta:
                table_meta[row.table_name]["row_count"] = row.total_rows
                table_meta[row.table_name]["size_bytes"] = row.total_logical_bytes
    except Exception:
        for tname in list(table_meta.keys()):
            try:
                tref = client.get_table(f"{project_id}.{dataset_id}.{tname}")
                table_meta[tname]["row_count"] = tref.num_rows
                table_meta[tname]["size_bytes"] = tref.num_bytes
            except Exception:
                pass
